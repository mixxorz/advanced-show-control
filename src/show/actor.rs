        ...
        while let Some(cmd) = rx.recv().await {
            match cmd {
                ...
                ShowCommand::SetLockout { enabled, reply } => {
                    ...
                    if changed {
                        event_bus.publish(AppEvent::Show(ShowEvent::LockoutChanged { enabled }));
                    }
                }
                ShowCommand::SetSceneDuration {
                    ...
                    let result = state.set_scene_duration_ms(&scene_id, duration_ms);
                ...
                ShowCommand::SetChannelScoped {
                    ...
                    let result = state.set_channel_scoped(&scene_id, group, channel, scoped);
                    if matches!(result, Ok(true)) {
                        event_bus
                            .publish(AppEvent::Show(ShowEvent::SceneConfigChanged { scene_id }));
                    }
                ...
                ShowCommand::SetAllChannelsScoped {
                    ...
                    let result = state.set_all_channels_scoped(&scene_id, scoped);
                    if matches!(result, Ok(true)) {
                        event_bus
                            .publish(AppEvent::Show(ShowEvent::SceneConfigChanged { scene_id }));
                    }
                ...
                ShowCommand::StoreSceneConfig {
                    ...
                    let result = state.store_scene_config(&scene_id, &channels);
                    if matches!(result, Ok(true)) {
                        event_bus
                            .publish(AppEvent::Show(ShowEvent::SceneConfigChanged { scene_id }));
                    }
                ...
                ShowCommand::ReplaceSnapshot { snapshot, reply } => {
                    state.replace_snapshot(snapshot);
                    let _ = reply.send(());
                }
                ShowCommand::Clear { reply } => {
                    state.clear();
                    let _ = reply.send(());
                }
            }
