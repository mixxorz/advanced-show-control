...
`AppRoot` owns one presentation-only pending destructive session action. Close, Quit, New, New from Template, and Open first request narrow authoritative dirty/path state from Show through the serial UI command lane, then use the same Save/Discard/Cancel flow. Query and save completion are correlated by command ID. A successful save triggers another authoritative Show query; the pending action continues only when that result is clean. Prompt cancellation, Save As cancellation, query/save failure, or edits observed during save clear the intent without replacing the session or closing the app. The platform close hook always vetoes the immediate close and exits only through a guarded Quit continuation.
