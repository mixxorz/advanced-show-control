...
`AppRoot` owns one presentation-only pending destructive session action. All native UI persisted scene/cue edits and lockout changes share one serial command lane with file operations. Close, Quit, New, New from Template, and Open therefore request narrow authoritative dirty/path state from Show only after earlier submitted persisted edits complete, then use the same Save/Discard/Cancel flow. Query and save completion are correlated by command ID. A successful save triggers another authoritative Show query; the pending action continues only when that result is clean. Prompt cancellation, Save As cancellation, query/save failure, or edits observed during save clear the intent without replacing the session or closing the app. The platform close hook always vetoes the immediate close and exits only through a guarded Quit continuation. A close request received while a native prompt, native dialog, or app modal is already active is vetoed without starting another preflight.
