...
The open connection dialog automatically measures each continuously present discovered identity once per presentation session, including identities discovered after opening; an identity pruned after disappearance receives a new measurement if it reappears. GPUI owns the session's pending/result state and assigns each dispatch a unique attempt ID, rejecting closed, removed, duplicate, superseded-attempt, or stale-session completions. Each measurement runs through the detached TCP-connect probe on Tokio; it does not use lifecycle connection transitions, generations, runtime peers, LV1 registration, or fader commands.
