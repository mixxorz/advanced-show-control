...
`AppRoot` owns one presentation-only pending destructive session action. All native UI persisted scene/cue edits and lockout changes use a dedicated submission method that increments a dispatcher-owned epoch before entering the serial command lane shared with file operations. Independently, `AppEventBus` maintains an owner-side persisted-session revision and a small shared admission gate across all clones. Persisted Scenes and Cue Lists publication, including LV1 reconciliation and GO cue advancement, and accepted Show lockout changes serialize revision increments on this gate; projection-only facts, session replacement, and unrelated domains do not advance it. Close, Quit, New, New from Template, and Open capture the UI epoch and receive the owner revision with narrow authoritative dirty/path state from Show. If either value differs from its current value when the GPUI callback handles a preflight or post-save result, the typed result is stale; `AppRoot` retains the guard phase and queues a fresh query behind the serial lane. A clean result also carries its exact owner revision through delayed file selection and asynchronous load preparation. Guarded New, Open, and New from Template compare it at the Scenes commit and hold the same gate across bounded in-memory scene/cue replacement and `SessionReplaced` publication; guarded Quit compares it while invoking GPUI quit under the gate. Revision mismatch cancels replacement without changing documents or Show metadata. Explicit Discard remains unconditional. Query and save completion are correlated by command ID. A successful save triggers another authoritative Show query; the pending action continues only when that result is clean. Prompt cancellation, Save As cancellation, query/save failure, or edits observed during save clear the intent without replacing the session or closing the app. The platform close hook always vetoes the immediate close and exits only through a guarded Quit continuation. Session actions are blocked while a guard is pending or a native prompt, native dialog, or app modal is active. A close request in those modal states is vetoed without starting another preflight. If another modal appears while an asynchronous guard query is pending, its result cancels the guarded action with a visible error instead of opening a nested prompt or continuing behind the modal.
...
`Show` does not own scene configs, selection, clipboard, or cue-list documents. It owns show-file path/name, dirty state, save timestamp, lockout, discovery, and connected-LV1 metadata. Scenes distinguishes persisted edits from projection-only updates; every Cue Lists change is a persisted edit. Show subscribes during construction, so edits cannot fall into a gap before its task starts. It observes these app-lifetime facts without generation filtering, marks dirty, and publishes its full projection without redundant reason tags. While one new, save, or load operation is pending, Show continues consuming facts, generation-fenced connection-metadata commands, and read-only state/path commands so lifecycle connection transitions are not blocked by persistence. Before replying to the narrow session-state query, Show non-blockingly consumes the event-bus facts that were already queued, so already-published persisted edits are reflected in its dirty result without delaying later lifecycle work. At the first other command, Show holds that single command and stops polling the mailbox until the operation completes; later safe commands cannot bypass the conflicting command. Persisted edits observed during the operation remain dirty after completion. On Show event-bus lag it conservatively marks the file dirty.
...
GPUI owns the native event loop, windows, focus, input, rendering, and transient navigation intent. A dedicated Tokio runtime owns actors, LV1 networking, timers, and asynchronous file work. GPUI context types do not enter actors, and Tokio runtime types do not enter component rendering. Handlers submit explicit commands without blocking the GPUI thread; the native bridge schedules accepted projection updates on the GPUI thread.

The custom connection modal is centered over a dim, interaction-blocking backdrop and owns a named focus trap until Escape or its close control restores the prior action focus. It automatically measures discovered identities, including identities discovered after opening, with at most one detached probe per identity in flight within a presentation session. GPUI owns separate displayed latency and in-flight ownership, and assigns each dispatch a unique attempt ID. Disappearance prunes displayed state but retains the in-flight slot, so reappearance before completion restores the same pending attempt without overlapping another probe. A completion received while absent clears that slot without restoring display state, allowing a fresh attempt if the identity later reappears. Closing clears both stores, and exact session, identity, and attempt matching rejects duplicate, superseded, or stale completions. Each measurement runs through the detached TCP-connect probe on Tokio; it does not use lifecycle connection transitions, generations, runtime peers, LV1 registration, or fader commands.
