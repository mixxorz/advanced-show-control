...
Store-from-current-LV1 uses the Scenes actor's single pending snapshot slot. The actor keeps consuming AppEventBus facts, lockout changes, and recall deadlines while that fresh LV1 state request is outstanding; scene and cue command mailboxes remain serialized until it completes. Disconnect or generation change drops the request and rejects the store before a late reply can mutate the document. Lag recovery synchronously fails closed, then polls its Settings refresh and optional fresh LV1 snapshot as one actor-owned pending operation alongside runtime, LV1, Settings, and lockout inputs. Generation change, disconnect, or a newer lag cancels stale recovery; revision fences prevent completion from overwriting newer Settings or scene-list facts. Abort All synchronously cancels coordinated recall intent, then polls Fade mailbox admission and acknowledgement as an actor-owned pending operation while facts, lockout, and recall deadlines remain responsive; scene and cue command mailboxes remain FIFO-gated, and stale completion cannot report success after generation change or disconnect. Explicit-recall admission and FIFO dispatch use synchronous coordinator-issued tokens while the actor polls each fresh LV1 snapshot and LV1 recall acknowledgement; cancellation consumes the current token and queued intent before late replies can commit or report success. At the settled-observation boundary, Scenes takes the coordinator's observation token and polls the current Settings request as an actor-owned pending phase, without blocking runtime, LV1, Settings, lockout, or recall-deadline inputs; newer Settings facts supersede the request result, and safety cancellation drops the token before continuation. After synchronous observation prechecks, exact fresh LV1 snapshot acquisition and retries run as a second actor-owned pending phase with the same responsive inputs and safety cancellation. The coordinator then issues a prepared Fade-handoff token; Fade mailbox reservation, generation-fenced publication and send, acknowledgement, and queued readiness-receiver transfer run as a final actor-owned pending phase. Scenes has no remaining serial external mailbox waits, and stale handoff completion cannot publish, commit, or report current success.
