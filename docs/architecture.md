...
The open connection dialog automatically measures each discovered identity once per presentation session, including identities discovered after opening. GPUI owns the session's pending/result state and rejects closed, removed, duplicate, or stale-session completions. Each measurement runs through the detached TCP-connect probe on Tokio; it does not use lifecycle connection transitions, generations, runtime peers, LV1 registration, or fader commands.
