...
`AppRoot` owns one presentation-only pending destructive session action. Projected Show dirty/path state gates close, Quit, New, New from Template, and Open through the same Save/Discard/Cancel flow. Save completion is correlated by command ID; only success or explicit discard continues the pending action. Prompt cancellation, Save As cancellation, and save failure clear the intent without replacing the session or closing the app. The platform close hook vetoes dirty closes until this flow resolves.
