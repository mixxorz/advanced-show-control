...
/// supplied platform app-config directory, a human-readable stdout sink, and the native UI sink.
/// Subject to the process-wide environment filter, the file and stdout
/// sinks MUST admit `DEBUG` and above at bootstrap, while the native UI sink MUST admit only `INFO`
...
/// A native UI log event MUST contain only its mapped severity and the tracing `message`, falling
