...
impl ConnectionState {
    ...
    pub fn begin_latency(&mut self, identity: &Lv1SystemIdentity) -> bool {
    ...
    div()
            ...
            div()
                ...
                .when(!unavailable && !is_connected && pending.is_none(), |row| {
                    row.on_click(move |_, window, _| {
                        ...
                        drop(state);
                        select_dispatcher.dispatch(move |commands| async move {
        ...
        .child(
            ...
            .on_click(move |_, window, _| {
                if probe_state.borrow_mut().begin_latency(&probe_identity) {
                    dispatcher.probe_latency(probe_identity.clone(), None);
                    window.refresh();
                }
            }),
        )
