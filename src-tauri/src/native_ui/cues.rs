...
use gpui_kit::{
    AppContext, Context, Entity, MouseButton, Render, SharedString, Window, div, prelude::*, px,
    rgb,
};
use uuid::Uuid;
...
impl CueListsView {
    ...
    fn render_manage_row(&self, list: CueList, cx: &mut Context<Self>) -> gpui_kit::AnyElement {
        ...
        div()
            ...
            .on_drop(move |payload: &CueListDrag, _, cx| {
                ...
                drop_entity.update(cx, |this, cx| {
                    if let Some(ids) = reordered_ids(&this.snapshot.cue_lists, from, id) {
                        this.dispatch(move |commands| {
                            Box::pin(
                                async move { commands.reorder_cue_lists(ids).await.map(|_| ()) },
                            )
                        });
                    }
                    cx.notify();
            ...
            .child(div().text_color(rgb(theme::CONSOLE_MUTED)).child("↕"))
            .child(
                div()
                    .id(format!("activate-cue-list-{id}"))
                    .flex_1()
                    .cursor_pointer()
                    .text_color(rgb(theme::CONSOLE_PRIMARY))
                    .child(name)
                            ...
                            .on_click(move |_, _, cx| {
                                entity.update(cx, |this, cx| {
                                    ...
                                    let command_id = match editor {
                                        NameEditor::Create => this.dispatch(move |commands| {
                                            Box::pin(async move {
                                                commands.create_cue_list(name).await.map(|_| ())
                                            })
                                        }),
                                        NameEditor::Rename(id) => this.dispatch(move |commands| {
                                            Box::pin(async move {
                                                commands.rename_cue_list(id, name).await.map(|_| ())
                                            })
                                        }),
                                    };
                                    this.pending_manage_command =
                                        Some((command_id, ManageCommand::Name(editor)));
                                    cx.notify();
                                });
                            }),
                    ),
...
impl Render for CueListsView {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let go_enabled = valid_cued_entry(&self.snapshot).is_some();
        let go_entity = cx.entity();
        div()
            .relative()
            .size_full()
            .p_3()
            ...
            .child(
                div()
                    ...
                    .child(
                        self.snapshot
                            .last_cue_recall_status
                            .clone()
                            .unwrap_or_default(),
                    )
                    .child(
                        Button::new("go-cued-entry")
                            .large()
                            .primary()
                            .label("GO")
                            .disabled(!go_enabled)
