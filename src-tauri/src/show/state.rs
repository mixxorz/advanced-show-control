...
impl ShowState {
    ...
    pub fn store_scene_config(
        ...
        let current_refs: Vec<crate::scenes::ChannelRef> = channels
            .iter()
            .map(|channel| crate::scenes::ChannelRef {
                group: channel.group,
    ...
    pub fn set_channel_scoped(
        ...
        match (scoped, ref_exists) {
            (true, false) => {
                scene
                    .scoped_channels
                    .push(crate::scenes::ChannelRef { group, channel });
                Ok(true)
            }
            (false, true) => {
                scene
    ...
    pub fn set_all_channels_scoped(
        ...
        let scene = self
            ...
            .ok_or_else(|| "Scene config not found".to_string())?;
        let mut changed = false;
        let refs: Vec<crate::scenes::ChannelRef> = scene
            .channel_configs
            .iter()
            .map(|entry| crate::scenes::ChannelRef {
                group: entry.group,
