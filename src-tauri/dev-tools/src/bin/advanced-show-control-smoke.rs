...
use advanced_show_control::native_ui::NativeRuntime;
...
fn main() {
    ...
    let mut runtime = match NativeRuntime::build() {
        Ok(runtime) => runtime,
        Err(error) => {
    ...
    let result = runtime.handle().block_on(run_lifecycle(
        commands,
        debug_commands,
        projections,
        &mut report,
...
async fn run_lifecycle(
    ...
    if let Some(error) = suite.first_failure() {
        let _ = runner.report.line(format!("ERROR {error}"));
    }
    ...
    if let Err(error) = runner.report.line(final_line) {
